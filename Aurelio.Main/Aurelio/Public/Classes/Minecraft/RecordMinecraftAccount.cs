using System.Collections.Generic;
using System.ComponentModel;
using System.Runtime.CompilerServices;
using Aurelio.Public.Module.App;
using Aurelio.Public.Module.Value;
using Avalonia.Media.Imaging;
using LiteSkinViewer2D;
using LiteSkinViewer2D.Extensions;
using Newtonsoft.Json;
using ReactiveUI;
using ReactiveUI.Fody.Helpers;
using SkiaSharp;

namespace Aurelio.Public.Classes.Minecraft;

public sealed class RecordMinecraftAccount : ReactiveObject, INotifyPropertyChanged
{
    [JsonIgnore] private Bitmap? _body;
    [JsonIgnore] private Bitmap? _head;
    [JsonProperty] [Reactive] public Enum.Setting.AccountType AccountType { get; set; }
    [JsonProperty] [Reactive] public string Name { get; set; } = "Unnamed";
    [JsonProperty] [Reactive] public string UUID { get; set; }
    [JsonIgnore] public string FormatLastUsedTime => Calculator.FormatUsedTime(LastUsedTime);
    [JsonProperty] [Reactive] public DateTime LastUsedTime { get; set; } = DateTime.MinValue;
    [JsonProperty] [Reactive] public DateTime AddTime { get; set; } = DateTime.MinValue;
    [JsonProperty] [Reactive] public string? Data { get; set; }

    [JsonProperty]
    [Reactive]
    public string Skin { get; set; } =
        @"";

    [JsonIgnore]
    public Bitmap Head
    {
        get { return _head ??= HandleHeadSkin(); }
    }

    [JsonIgnore]
    public Bitmap Body
    {
        get { return _body ??= HandleBodySkin(); }
    }

    public RecordMinecraftAccount()
    {
        PropertyChanged += (_, _) =>
        {
            AppMethod.SaveSetting();
        };
    }
    
    public override bool Equals(object? obj)
    {
        if (obj is not RecordMinecraftAccount other) return false;
        return Skin == other.Skin && Name == other.Name && Data == other.Data;
    }

    private Bitmap HandleBodySkin()
    {
        var imageBytes = Convert.FromBase64String(Skin);
        return FullBodyCapturer.Default.Capture(SKBitmap.Decode(imageBytes)).ToBitmap();
    }
    
    private Bitmap HandleHeadSkin()
    {
        var imageBytes = Convert.FromBase64String(Skin);
        return HeadCapturer.Default.Capture(SKBitmap.Decode(imageBytes)).ToBitmap(48);
    }
}