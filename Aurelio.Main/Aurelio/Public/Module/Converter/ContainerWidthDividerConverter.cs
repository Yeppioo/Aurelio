...
public class ContainerWidthDividerConverter : IMultiValueConverter
{
    public object Convert(IList<object> values, Type targetType, object parameter, CultureInfo culture)
    {
        ...
        else if (values[0] is string widthStr && double.TryParse(widthStr, out var parsedWidth))
        ...
        {
            if (int.TryParse(countStr, out var parsedCount) && parsedCount > 0)
                count = parsedCount;
            else if (double.TryParse(countStr, out var parsedDblCount) && parsedDblCount > 0)
                count = (int)parsedDblCount;
        }

        var itemWidth = Math.Floor(containerWidth / count - 10); // 减去边距，取整
