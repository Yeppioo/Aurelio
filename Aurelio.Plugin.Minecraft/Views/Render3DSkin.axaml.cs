...
using LiteSkinViewer.Shared.Enums;
using PointerType = LiteSkinViewer.Shared.Enums.PointerType;
...
public partial class Render3DSkin : UserControl, IAurelioTabPage
{
    ...
    public Render3DSkin(string name, string base64)
    {
        ...
        Loaded += async (_, _) =>
        {
            if (!_fl) return;
            _fl = false;
            skinViewer.Skin = Public.Module.Value.Converter.Base64ToBitmap(base64);
