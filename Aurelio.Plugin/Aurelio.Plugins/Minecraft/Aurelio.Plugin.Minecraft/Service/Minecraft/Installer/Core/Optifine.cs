...
{
    public static async Task<bool> Main(OptifineInstallEntry entry, string path, string jPath, string customId,
        TaskEntry mainTask, TaskEntry task1, TaskEntry task2, CancellationToken cancellationToken)
    {
        try
        {
            var installer = OptifineInstaller.Create(path, jPath, entry, customId);
