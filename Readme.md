...
- **🪟 标签页拖拽分离**: 将任意标签页拖拽到新窗口中独立运行
- **🔄 标签页重排序**: 通过拖拽轻松调整标签页顺序
- **⚡ 快速切换**: 在不同游戏实例间瞬间切换
- **🎯 独立配置**: 每个标签页拥有独立的游戏配置
- **💾 状态保存**: 自动保存标签页布局和状态
...
- **📦 版本管理**: 支持原版、Forge、Fabric、Quilt 等所有主流版本
- **☕ Java 管理**: 自动检测和管理多版本 Java 环境
- **🚀 快速启动**: 优化的启动流程，减少等待时间
- **📊 实时监控**: 游戏进程状态实时监控
- **🔧 启动参数**: 高级启动参数自定义
...
- **📁 Mod 管理**: 一键启用/禁用 Mod，支持批量操作
- **🔍 Mod 信息**: 显示 Mod 详细信息和依赖关系
- **📥 便捷安装**: 拖拽安装 Mod 文件
- **⚠️ 冲突检测**: 智能检测 Mod 冲突和兼容性
- **🏷️ 分类管理**: 按类别组织和管理 Mod
...
1. **📂 创建标签页**: 点击 "+" 按钮创建新的游戏标签页
2. **⚙️ 配置实例**: 选择版本、Mod、资源包等
3. **🚀 启动游戏**: 点击启动按钮开始游戏
4. **🪟 多窗口运行**: 拖拽标签页到新窗口实现多开
